...
We have a few new packages today. Those include noaa and leaflet. nooa is a package used to download NOAA climate data. leaflet is a package for interactive mapping.

Recall that if you have not installed these packages on your computer you will need to run:

install.packages("noaa") and so on for the others. 
...
First, let's start by getting some precipitation (P) data. noaa has a function called ghcnd_stations(). This function will download the site information for all stations in the GHCND network. 
