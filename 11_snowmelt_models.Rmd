...
These models prioritize pattern reproduction. In other words, these work well when we don't care why snow melts, just that it does. These models use historical data and statistical techniques to predict runoff based on the relationship between snow characteristics (like snow area) and runoff. They use past patterns to make predictions about the future. The emergence of data-driven models has benefited from the growth of massive data and the rapid increase in computational power. These models simulate the changes in snowmelt runoff using machine learning algorithms to select appropriate parameters (e.g., daily rainfall, temperature, solar radiation, snow area, and snow water equivalent) from different data sources.

**Conceptual Models:** 
These models simplify the snowmelt process by establishing a simple, rule-based relationship between snowmelt and temperature. You may also see these refereed to as temperature index models as they prioritize air temperature as a proxy for energy. These models use a basic formula based on temperature to estimate how much snow will melt. These models work because air temperature can serve as a proxy for multiple energy fluxes (like radiation or sensible heat, etc.), even though it does not explicitly represent those processes. They serve as a nice example of how models are 'wrong but useful'. 
          
**Physical Models:** 
The physical snowmelt models calculate snowmelt based on the energy balance of snow cover (i.e. they prioritize energy fluxes). If all the heat fluxes toward the snowpack are considered positive and those away are considered negative, the sum of these fluxes is equal to the change in heat content of the snowpack for a given time period. Fluxes considered may be
...
We may also identify models based on the model architecture or spatial complexity. The architecture can be designed based on assumptions about the physical processes that may affect the snowmelt to runoff volume and timing. Therefore the choice of spatial complexity should reflect 1. the scale of interest (i.e., plot vs watershed?) 2. data availability and 3. whether timing or total volume is important. Many snowmelt models exhibit tradeoffs between accurately predicting the timing of melt onset and peak flow versus the total volume of runoff, and model selection often depends on which outcome is more important.
...
### [Repo link](https://github.com/tpcovino/06_snowmelt_models.git){target="_blank"}
