...
Webscraping in R is a method of extracting data from web pages when the data isn't available through an API or a direct download. There are many ways to do this, but this video will specifically help you locate NOAA weather data and show you how to estimate the distance of the available weather station from your area of interest using latitude and longitude [noaa webscraping example](https://www.youtube.com/watch?v=V5Df6vw4-e8). Even if your dataset isn't temperature related, you may still use this structure to extract the data you need. 

Pat Schloss has a lot more videos on data science tools in R that may be of interest to you as well.

Here is a condensed version of the linked video script up until ~18 min. You'll want to watch the video to understand how station to AOI distance is calculated. Once you find a site that works for you, you'll want to continue your own script where he starts working with glue().

```{r}
# Here we are sending a request to a webpage to retrievev its html content.
inventory_url <- 'https://www.ncei.noaa.gov/pub/data/ghcn/daily/ghcnd-inventory.txt'

inventory <- read_table(inventory_url, col_names = c("station", "lat", "lon", "variable", "start", "end"))

# we'll parse the data according to our desired location. Here we need to convert lat and lon of the Fool Creek watershed to radians so we can calculate distances between two latitude and longitudes so we can be sure we are selecting a site near our watershed.

my_lat <- 39.885 * 2 * pi / 360
my_lon <- -105.871* 2 * pi / 360


inventory %>%
  mutate(lat_r = lat* 2 * pi / 360,
         lon_r = lon* 2 * pi / 360,
         d = 1.609344 * 3963 * acos((sin(lat_r) * sin(my_lat)) + cos(lat_r) * cos(my_lat) * cos(my_lon-lon_r))) %>% # calculate distance bewteen our site and station in miles
  filter(start >= 2018 & end  > 2022) %>% # look for data after 2018
  filter(variable %in% c("TMAX", "TMIN")) %>%
  arrange(d) #find the station closest to us
```
