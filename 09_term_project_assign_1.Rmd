...
You are encouraged to explore many packages and write many more code chunks for your personal use. However, for the assignment submission, retrieve at least one dataset using an R package like dataRetrieval.
...
Generate plots:
Create at least one plot that summarizes the data and describes it's use to you. Are there gaps in the data? Does the data cover the time or space you are interested in? Are there significant outliers that need consideration? 
...
data_table <- data.frame(
  ...
  Data_Quality_notes = c('missing some dates', 'some unexpected values'),
  ...
  explain_response = c('explanatory', 'response')
)
